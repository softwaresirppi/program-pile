...
void generate(Tone tone, int bitrate, int volume){
	for(int i = 0; i < tone.snapshots; i++)
		little_endian_16(
			(sin(2 * PI * i * (tone.frequency/bitrate)) >= 0) * volume
		);
		//little_endian_16(
		//		normalize(
		//			sin(2 * PI * i * (tone.frequency/bitrate)),
		//			-1, 1
		//		) * ((1 << (8 * sample_bytes)) >> 1)
		//);
		//little_endian_16((int)round((sin(2 * PI * i * (tone.frequency/bitrate)) + 1) / 2) * 0x3FFF);
}

int main() {
	int CHANNELS = 1; // Nc
	int FREQUENCY = 44100;// F
	int SAMPLE_BYTES = 2; // M # of bytes in a sample across a specific channel and snapshot
	...
	for(int i = 0; i < sizeof(tones)/sizeof(Tone); i++)
		generate(tones[i], FREQUENCY, pow(2, 8 * SAMPLE_BYTES) / 2);
